...
class BurpExtender(IBurpExtender, IContextMenuFactory):
    ...
    def createMenuItems(self, context_menu):        #Required function (createMenuItems) when using 'IContextMenuFactory'
        self.context = context_menu
        menu_list = ArrayList()
        menu_list.add(JMenuItem('Send to Bing', actionPerformed=self.bing_menu))    #Name that pops up on request (calls bing menu function)
        return menu_list

    def bing_menu(self, event):                                     #Built in functions in ContextMenu
        http_traffic = self.context.getSelectedMessages()
        print('%d requests highlighted' % len(http_traffic))

        for traffic in http_traffic:
            http_service = traffic.getHttpService()                 #assigning to service variable
            host = http_service.getHost()
            print('User selected host: %s' % host)                  
            self.bing_search(host)                                  #Passes host to search
        return
    ...
    def bing_query(self, bing_query_string):
        ...
        http_request += 'User-Agent: blacklistedx\r\n\r\n'
